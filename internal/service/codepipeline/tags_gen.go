...
// listTags_ returns a function that lists codepipeline service tags and set them in Context.
...
func listTags_() types.ListTagsFunc {
	return func(ctx context.Context, meta any, identifier string) error {
		tags, err := listTags(ctx, meta.(*conns.AWSClient).CodePipelineConn(ctx), identifier)
...
// updateTags_ returns a function that updates codepipeline service tags.
// It is called by the transparent tagging interceptor.
func updateTags_() types.UpdateTagsFunc {
	return func(ctx context.Context, meta any, identifier string, oldTags, newTags any) error {
		return updateTags(ctx, meta.(*conns.AWSClient).CodePipelineConn(ctx), identifier, oldTags, newTags)
	}
}
