...
// listTags_ returns a function that lists ivs service tags and set them in Context.
...
func listTags_() types.ListTagsFunc {
	return func(ctx context.Context, meta any, identifier string) error {
		tags, err := listTags(ctx, meta.(*conns.AWSClient).IVSConn(ctx), identifier)
...
// updateTags_ returns a function that updates ivs service tags.
// It is called by the transparent tagging interceptor.
func updateTags_() types.UpdateTagsFunc {
	return func(ctx context.Context, meta any, identifier string, oldTags, newTags any) error {
		return updateTags(ctx, meta.(*conns.AWSClient).IVSConn(ctx), identifier, oldTags, newTags)
	}
}
