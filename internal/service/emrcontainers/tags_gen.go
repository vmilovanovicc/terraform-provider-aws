...
// listTags_ returns a function that lists emrcontainers service tags and set them in Context.
...
// updateTags_ returns a function that updates emrcontainers service tags.
// It is called by the transparent tagging interceptor.
func updateTags_() types.UpdateTagsFunc {
	return func(ctx context.Context, meta any, identifier string, oldTags, newTags any) error {
		return updateTags(ctx, meta.(*conns.AWSClient).EMRContainersConn(ctx), identifier, oldTags, newTags)
	}
}
